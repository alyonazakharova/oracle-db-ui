...
SELECT * FROM JOURNAL WHERE DATE_RET < '15-03-21';


--Посчитать количество книг, которых нет в наличии.
SELECT COUNT(*) FROM BOOKS WHERE CNT=0;
...
SELECT COUNT(*) FROM JOURNAL WHERE CLIENT_ID=1 AND DATE_RET IS NULL;
...
(SELECT TRUNC(DATE_RET) - TRUNC(DATE_END) AS DAYS, FINE
    ...
    WHERE DATE_RET IS NOT NULL AND J.DATE_RET > J.DATE_END AND CLIENT_ID=1);
